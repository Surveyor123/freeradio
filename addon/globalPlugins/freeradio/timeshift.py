...
class TimeShiftBuffer:
    ...
    def _strip_icy_metadata(self, data):
        ...
        while remaining:
            if self._icy_bytes_until_meta == 0:
                ...
                if meta_len > 0:
                    if len(remaining) >= meta_len:
                        remaining = remaining[meta_len:]
                    else:
                        break
    ...
    def _run_once(self, my_gen):
        ...
        # If we have a prefix from HTTPS socket or ICY fallback, write it
        if prefix and not self._is_stale(my_gen) and not is_socket:
            clean_prefix = self._strip_icy_metadata(prefix)
            if clean_prefix:
                self._maybe_capture_flac_header(clean_prefix)
                self._write_chunk(clean_prefix, my_gen)
